...
import {questions,decide} from './policy-v2.mjs';
...
const old=await read('sample.json'),labels=(await read('labels.json')).items;
const challenges=await read('challenge-v2.json');
const cases=[...old.map(x=>({...x,set:'original',expected:labels.find(l=>l.id===x.id).action})),...challenges];
...
for(const item of cases){
 ...
 await writeFile(new URL('results-v2.json',dir),JSON.stringify(rows,null,2)+'\n');
...
const metrics={original:summarize(rows.filter(r=>r.set==='original')),syntheticChallenge:summarize(rows.filter(r=>r.set==='synthetic')),inputTokens:tokens,estimatedUSD:tokens*42/1e9,medianMs:lat[Math.floor(lat.length/2)],p95Ms:lat[Math.ceil(lat.length*.95)-1]};
await writeFile(new URL('metrics-v2.json',dir),JSON.stringify(metrics,null,2)+'\n');console.log(JSON.stringify(metrics,null,2));
