...
export const configured = () => ['UPSTASH_REDIS_REST_URL','UPSTASH_REDIS_REST_TOKEN','VAPID_PUBLIC_KEY','VAPID_PRIVATE_KEY','VAPID_SUBJECT','CRON_SECRET','APP_ORIGIN','PUSH_ENROLLMENT_KEY'].every(k => Boolean(process.env[k]));
...
export async function redis(...command) {
  ...
  const response = await fetch(url, { method: 'POST', redirect: 'error', signal: AbortSignal.timeout(8000), headers: { Authorization: `Bearer ${process.env.UPSTASH_REDIS_REST_TOKEN}`, 'Content-Type': 'application/json' }, body: JSON.stringify(command) });
